...
OUTPUT_DIR = PLOT_ROOT / "result_display" / "test10_temperature_curves" / "difftemp"
...
def process_point(args: argparse.Namespace, point_name: str, data_dir: Path) -> dict[str, object] | None:
    ...
    process_path = args.output_dir / "csv" / f"10_{point_name}_temperature_process.csv"
    results_path = args.output_dir / "csv" / f"10_{point_name}_temperature_results.csv"
    failures_path = args.output_dir / "csv" / f"10_{point_name}_processing_failures.csv"
    write_csv(
        process_path,
        trial_process_rows(
    ...
    write_csv(results_path, trial_result_rows(trials, args.cold_junction_c))
    write_csv(failures_path, failures)
    ...
    signed_normalized_dir = args.output_dir / "png" / "pun_normalized_have_signal"
    signed_normalized_dir.mkdir(parents=True, exist_ok=True)
    signed_normalized_path = signed_normalized_dir / f"10_{point_name}_signed_normalized_response.png"
    fig_signed_normalized.savefig(signed_normalized_path)
    ...
    print(f"Saved process table: {process_path}")
...
def main() -> None:
    ...
    for point_name, data_dir in point_dirs:
        ...
        all_result_rows.extend(trial_result_rows(result["trials"], args.cold_junction_c))
