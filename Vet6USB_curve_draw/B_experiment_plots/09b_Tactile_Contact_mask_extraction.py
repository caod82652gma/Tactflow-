"""
Batch tactile contact-mask extraction for VET6USB 6x6 CSV exports.

conda activate vet6usb_pyqt
python Vet6USB_curve_draw/B_experiment_plots/09b_Tactile_Contact_mask_extraction.py --mode force-rise
...
FORCE_OUTPUT_DIR = RESULT_DISPLAY_ROOT / "test9b_tactile_contact_mask_extraction"
...
CONTACT_STABILITY_WINDOW_MS = 50.0
CONTACT_STABILITY_MAX_CHANGED_TAXELS = 2
...
class ContactMaskTrial:
    ...
    peak_idx: int
    mask: np.ndarray
    peak_frame_mask: np.ndarray
    max_abs_map: np.ndarray
    peak_abs_map: np.ndarray
    active_taxels: int
    coverage_pct: float
    centroid_row: float
    centroid_col: float
    bbox_rows: str
    bbox_cols: str
    peak_envelope_lsb: float
    baseline_level_lsb: float
    threshold_lsb: float
    t_stable_idx: int
    t_stable_ms: float
    stable_consistency_pct: float
    stable_remaining_changed_taxels: int
    stability_window_ms: float
    stability_window_samples: int
    stability_max_changed_taxels: int
    error_message: str = ""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Process VET6USB tactile contact-mask CSV trials.")
    ...
    parser.add_argument("--contact-summary-n", type=int, default=SELECT_N_PER_DISTANCE)
    ...
    parser.add_argument("--contact-stability-window-ms", type=float, default=CONTACT_STABILITY_WINDOW_MS)
    parser.add_argument(
        "--contact-stability-max-changed-taxels",
        type=int,
        default=CONTACT_STABILITY_MAX_CHANGED_TAXELS,
    )
...
def update_runtime_config(args: argparse.Namespace) -> None:
    ...
    global CONTACT_STABILITY_WINDOW_MS, CONTACT_STABILITY_MAX_CHANGED_TAXELS
    FS = float(args.fs)
    ...
    CONTACT_STABILITY_WINDOW_MS = float(args.contact_stability_window_ms)
    CONTACT_STABILITY_MAX_CHANGED_TAXELS = int(args.contact_stability_max_changed_taxels)
...
def mask_bbox(mask: np.ndarray) -> tuple[str, str]:
    rows, cols = np.where(mask)
    if rows.size == 0:
        return "", ""
    return f"{int(rows.min())}-{int(rows.max())}", f"{int(cols.min())}-{int(cols.max())}"


def mask_centroid(mask: np.ndarray, weights: np.ndarray) -> tuple[float, float]:
    ...
    if total <= EPS:
        ...
        total = float(np.sum(active_weights))
    return (
        float(np.sum(row_grid * active_weights) / total),
        float(np.sum(col_grid * active_weights) / total),
    )


def contact_stability_window_samples(window_ms: float) -> int:
    return max(1, int(math.ceil(window_ms / 1000.0 * FS)))


def compute_contact_stability(
    abs_frames: np.ndarray,
    final_mask: np.ndarray,
    threshold_lsb: float,
    window_ms: float,
    max_changed_taxels: int,
) -> tuple[int, float, int, int]:
    window_samples = contact_stability_window_samples(window_ms)
    if abs_frames.shape[0] == 0:
        return 0, float("nan"), N_ROWS * N_COLS, window_samples

    cumulative_masks = np.maximum.accumulate(abs_frames, axis=0) >= threshold_lsb
    final_bool = final_mask.astype(bool)
    changed_counts = np.count_nonzero(cumulative_masks != final_bool, axis=(1, 2))
    n_frames = changed_counts.size

    for idx in range(n_frames):
        stop = min(n_frames, idx + window_samples)
        if np.max(changed_counts[idx:stop]) <= max_changed_taxels:
            changed_taxels = int(changed_counts[idx])
            consistency_pct = float((N_ROWS * N_COLS - changed_taxels) / (N_ROWS * N_COLS) * 100.0)
            return idx, consistency_pct, changed_taxels, window_samples
...
def process_contact_mask_trial(
    csv_path: Path,
    distance_label: str,
    apply_blur: bool,
    gaussian_sigma: float,
    ...
    peak_level = float(smooth_envelope[peak_idx])
    if peak_level <= baseline_level:
        raise ValueError("no contact peak above baseline")
    ...
    hits = np.flatnonzero(smooth_envelope[: peak_idx + 1] >= onset_threshold)
    if hits.size == 0:
        raise ValueError("no contact onset detected before peak")

    t_start = int(hits[0])
    window_stop = min(len(X_filtered), peak_idx + 1)
    if window_stop <= t_start:
        window_stop = min(len(X_filtered), t_start + 1)
    contact_window = np.abs(X_filtered[t_start:window_stop])
    if contact_window.size == 0:
        raise ValueError("empty contact window")

    max_abs_map = np.max(contact_window, axis=0)
    mask = max_abs_map >= threshold_lsb
    stable_offset_idx, stable_consistency_pct, stable_changed_taxels, stable_window_samples = compute_contact_stability(
        abs_frames=contact_window,
        final_mask=mask,
        threshold_lsb=threshold_lsb,
        window_ms=CONTACT_STABILITY_WINDOW_MS,
        max_changed_taxels=CONTACT_STABILITY_MAX_CHANGED_TAXELS,
    )
    t_stable_idx = t_start + stable_offset_idx
    t_stable_ms = stable_offset_idx / FS * 1000.0
    peak_abs_map = np.abs(X_filtered[peak_idx])
    peak_frame_mask = peak_abs_map >= threshold_lsb
    active_taxels = int(np.count_nonzero(mask))
    coverage_pct = float(active_taxels / (N_ROWS * N_COLS) * 100.0)
    centroid_row, centroid_col = mask_centroid(mask, max_abs_map)
    bbox_rows, bbox_cols = mask_bbox(mask)

    return ContactMaskTrial(
        path=csv_path,
        distance_label=distance_label,
        label=make_label(csv_path),
        t_start=t_start,
        ...
        peak_frame_mask=peak_frame_mask.astype(np.uint8),
        max_abs_map=max_abs_map,
        peak_abs_map=peak_abs_map,
        ...
        bbox_rows=bbox_rows,
        bbox_cols=bbox_cols,
        peak_envelope_lsb=peak_level,
        baseline_level_lsb=baseline_level,
        threshold_lsb=float(threshold_lsb),
        t_stable_idx=t_stable_idx,
        t_stable_ms=t_stable_ms,
        stable_consistency_pct=stable_consistency_pct,
        stable_remaining_changed_taxels=stable_changed_taxels,
        stability_window_ms=float(CONTACT_STABILITY_WINDOW_MS),
        stability_window_samples=stable_window_samples,
        stability_max_changed_taxels=int(CONTACT_STABILITY_MAX_CHANGED_TAXELS),
    )


def batch_process_contact_masks(
    trial_paths_dict: dict[str, list[Path]],
    apply_blur: bool,
    gaussian_sigma: float,
    threshold_lsb: float,
) -> tuple[dict[str, list[ContactMaskTrial]], list[dict[str, object]]]:
    results: dict[str, list[ContactMaskTrial]] = {group: [] for group in trial_paths_dict}
    failures: list[dict[str, object]] = []
    for distance_label, csv_path in iter_grouped_force_csvs(trial_paths_dict):
        try:
            results.setdefault(distance_label, []).append(
                process_contact_mask_trial(
                    csv_path=csv_path,
                    distance_label=distance_label,
                    apply_blur=apply_blur,
                    gaussian_sigma=gaussian_sigma,
...
def contact_mask_summary_table(results: dict[str, list[ContactMaskTrial]]) -> pd.DataFrame:
    rows = []
    for distance_label in FORCE_DISTANCE_GROUPS:
        for trial in results.get(distance_label, []):
            rows.append(
                {
                    ...
                    "t_start": trial.t_start,
                    "peak_idx": trial.peak_idx,
                    "active_taxels": trial.active_taxels,
                    "coverage_pct": trial.coverage_pct,
                    "centroid_row": trial.centroid_row,
                    "centroid_col": trial.centroid_col,
                    "bbox_rows": trial.bbox_rows,
                    "bbox_cols": trial.bbox_cols,
                    "peak_envelope_lsb": trial.peak_envelope_lsb,
                    "baseline_level_lsb": trial.baseline_level_lsb,
                    "threshold_lsb": trial.threshold_lsb,
                    "t_stable_idx": trial.t_stable_idx,
                    "t_stable_ms": trial.t_stable_ms,
                    "stable_consistency_pct": trial.stable_consistency_pct,
                    "stable_remaining_changed_taxels": trial.stable_remaining_changed_taxels,
                    "stability_window_ms": trial.stability_window_ms,
                    "stability_window_samples": trial.stability_window_samples,
                    "stability_max_changed_taxels": trial.stability_max_changed_taxels,
                }
            )
    return pd.DataFrame(rows)


def contact_mask_establishment_summary_table(
    results: dict[str, list[ContactMaskTrial]],
    max_trials_per_group: int | None,
) -> pd.DataFrame:
    rows = []
    for distance_label in FORCE_DISTANCE_GROUPS:
        available_trials = results.get(distance_label, [])
        if max_trials_per_group is not None and max_trials_per_group > 0:
            trials = available_trials[:max_trials_per_group]
        else:
            trials = available_trials
        stable_times = np.array([trial.t_stable_ms for trial in trials], dtype=float)
        stable_times = stable_times[np.isfinite(stable_times)]
        consistencies = np.array([trial.stable_consistency_pct for trial in trials], dtype=float)
        consistencies = consistencies[np.isfinite(consistencies)]

        n = int(stable_times.size)
        rows.append(
            {
                "distance": distance_label,
                "available_trials": int(len(available_trials)),
                "n_trials": n,
                "t_stable_mean_ms": float(np.mean(stable_times)) if n else float("nan"),
                "t_stable_std_1sigma_ms": float(np.std(stable_times, ddof=1)) if n > 1 else 0.0 if n == 1 else float("nan"),
                "t_stable_min_ms": float(np.min(stable_times)) if n else float("nan"),
                "t_stable_max_ms": float(np.max(stable_times)) if n else float("nan"),
                "stable_consistency_mean_pct": float(np.mean(consistencies)) if consistencies.size else float("nan"),
                "stability_window_ms": float(CONTACT_STABILITY_WINDOW_MS),
                "stability_window_samples": contact_stability_window_samples(CONTACT_STABILITY_WINDOW_MS),
                "stability_max_changed_taxels": int(CONTACT_STABILITY_MAX_CHANGED_TAXELS),
            }
        )
    return pd.DataFrame(rows)
...
def save_matrix_csv(matrix: np.ndarray, path: Path, fmt: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(
        matrix,
        index=[f"R{row}" for row in range(N_ROWS)],
        columns=[f"C{col}" for col in range(N_COLS)],
    ).to_csv(path, float_format=fmt, encoding="utf-8-sig")


def save_contact_mask_arrays(results: dict[str, list[ContactMaskTrial]], output_dir: Path) -> list[Path]:
    saved_paths: list[Path] = []
    matrix_root = output_dir / "process_csv" / "mask_csv"
    for distance_label in FORCE_DISTANCE_GROUPS:
        for trial in results.get(distance_label, []):
            trial_dir = matrix_root / distance_label
            stem = trial.path.stem
            paths = [
                trial_dir / f"{stem}_contact_mask.csv",
                trial_dir / f"{stem}_peak_frame_mask.csv",
                trial_dir / f"{stem}_max_abs_map_lsb.csv",
                trial_dir / f"{stem}_peak_abs_map_lsb.csv",
            ]
            save_matrix_csv(trial.mask, paths[0], fmt="%d")
            save_matrix_csv(trial.peak_frame_mask, paths[1], fmt="%d")
            save_matrix_csv(trial.max_abs_map, paths[2], fmt="%.6f")
            save_matrix_csv(trial.peak_abs_map, paths[3], fmt="%.6f")
            saved_paths.extend(paths)
    return saved_paths


def group_contact_frequency(results: dict[str, list[ContactMaskTrial]]) -> dict[str, np.ndarray]:
    frequencies: dict[str, np.ndarray] = {}
    for distance_label in FORCE_DISTANCE_GROUPS:
        trials = results.get(distance_label, [])
        if not trials:
            continue
        stack = np.stack([trial.mask.astype(float) for trial in trials], axis=0)
        frequencies[distance_label] = np.mean(stack, axis=0)
    return frequencies


def save_contact_frequency_tables(frequencies: dict[str, np.ndarray], output_dir: Path) -> list[Path]:
    saved_paths: list[Path] = []
    table_dir = output_dir / "process_csv" / "group_frequency_csv"
    for distance_label, matrix in frequencies.items():
        path = table_dir / f"{distance_label}_contact_frequency.csv"
        save_matrix_csv(matrix, path, fmt="%.6f")
        saved_paths.append(path)
    return saved_paths


def plot_single_contact_mask(trial: ContactMaskTrial, output_dir: Path) -> Path:
    fig, axes = plt.subplots(1, 2, figsize=(6.2, 2.9), constrained_layout=True)
    im0 = axes[0].imshow(trial.max_abs_map, cmap="viridis", origin="upper")
    axes[0].contour(trial.mask, levels=[0.5], colors="white", linewidths=1.0)
    axes[0].set_title("Max abs response")
    axes[0].set_xticks(range(N_COLS))
    axes[0].set_yticks(range(N_ROWS))
    fig.colorbar(im0, ax=axes[0], fraction=0.046, pad=0.04, label="LSB")

    axes[1].imshow(trial.mask, cmap="Greys", vmin=0, vmax=1, origin="upper")
    ...
    axes[1].text(
        0.02,
        -0.14,
        f"active={trial.active_taxels}/36, coverage={trial.coverage_pct:.1f}%",
        transform=axes[1].transAxes,
        ha="left",
        va="top",
    )
    fig.suptitle(f"{trial.distance_label}/{trial.path.name}")

    mask_dir = output_dir / "png" / "contact_masks" / trial.distance_label
    mask_dir.mkdir(parents=True, exist_ok=True)
    path = mask_dir / f"{trial.path.stem}_contact_mask.png"
    fig.savefig(path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    return path


def plot_group_contact_frequency(frequencies: dict[str, np.ndarray], output_dir: Path) -> str:
    available = [group for group in FORCE_DISTANCE_GROUPS if group in frequencies]
    if not available:
        return ""

    fig, axes = plt.subplots(1, len(available), figsize=(2.25 * len(available), 2.55), constrained_layout=True)
    if len(available) == 1:
        axes = [axes]
    im = None
    for ax, distance_label in zip(axes, available):
        im = ax.imshow(frequencies[distance_label], cmap="magma", vmin=0.0, vmax=1.0, origin="upper")
        ...
        for row in range(N_ROWS):
            for col in range(N_COLS):
                ax.text(
                    col,
                    row,
                    f"{frequencies[distance_label][row, col]:.0%}",
                    ha="center",
                    va="center",
                    fontsize=6.5,
                    color="white" if frequencies[distance_label][row, col] >= 0.45 else "black",
                )
    if im is not None:
        fig.colorbar(im, ax=list(axes), fraction=0.025, pad=0.02, label="Contact frequency")
    fig.suptitle("Contact-mask frequency by force distance")
    path = save_figure(fig, os.fspath(output_dir), "09b_contact_mask_frequency_by_distance")
    plt.close(fig)
    return path


def plot_group_contact_coverage(summary: pd.DataFrame, output_dir: Path) -> str:
    if summary.empty:
        return ""
    fig, ax = plt.subplots(figsize=(6.9, 3.5))
    groups = [group for group in FORCE_DISTANCE_GROUPS if group in set(summary["distance"])]
    data = [summary.loc[summary["distance"] == group, "coverage_pct"].to_numpy(dtype=float) for group in groups]
    ax.boxplot(data, tick_labels=groups, patch_artist=True)
    for idx, values in enumerate(data, start=1):
        if values.size:
            ax.scatter(np.full(values.size, idx), values, s=16, color=COLORS["blue"], alpha=0.55, zorder=3)
    ax.set_title("Contact-mask coverage distribution")
    ax.set_xlabel("Distance group")
    ax.set_ylabel("Active taxels (%)")
    ax.set_ylim(0.0, 100.0)
    path = save_figure(fig, os.fspath(output_dir), "09b_contact_mask_coverage_by_distance")
    plt.close(fig)
    return path


def save_contact_mask_figures(results: dict[str, list[ContactMaskTrial]], output_dir: Path) -> list[Path]:
    saved_paths: list[Path] = []
    for distance_label in FORCE_DISTANCE_GROUPS:
        for trial in results.get(distance_label, []):
            saved_paths.append(plot_single_contact_mask(trial, output_dir))
    return saved_paths


def run_contact_mask_pipeline(args: argparse.Namespace) -> None:
    ...
    if not any(paths for paths in trial_paths.values()):
        ...
        raise FileNotFoundError(f"No force-test CSV files found under:\n{searched}")

    results, failures = batch_process_contact_masks(
        trial_paths_dict=trial_paths,
        apply_blur=args.gaussian_blur,
        gaussian_sigma=args.gaussian_sigma,
        threshold_lsb=FIXED_ACTIVITY_THRESHOLD_LSB,
    )
    summary = contact_mask_summary_table(results)
    summary_path = args.force_output_dir / "contact_mask_summary_all_trials.csv"
    summary.to_csv(summary_path, index=False, encoding="utf-8-sig")
    establishment_summary = contact_mask_establishment_summary_table(
        results,
        max_trials_per_group=args.contact_summary_n,
    )
    establishment_summary_path = args.force_output_dir / "contact_mask_establishment_summary_by_group.csv"
    establishment_summary.to_csv(establishment_summary_path, index=False, encoding="utf-8-sig")
    failure_path = write_failures(failures, args.force_output_dir)

    matrix_paths = save_contact_mask_arrays(results, args.force_output_dir)
    frequencies = group_contact_frequency(results)
    frequency_paths = save_contact_frequency_tables(frequencies, args.force_output_dir)
    per_trial_figures = save_contact_mask_figures(results, args.force_output_dir)
    frequency_figure_path = plot_group_contact_frequency(frequencies, args.force_output_dir)
    coverage_figure_path = plot_group_contact_coverage(summary, args.force_output_dir)

    print("Contact-mask extraction summary")
    print("distance, csv_count, processed_count")
    for distance_label in FORCE_DISTANCE_GROUPS:
        print(
            f"{distance_label}, "
            f"{len(trial_paths.get(distance_label, []))}, "
            f"{len(results.get(distance_label, []))}"
        )
    print(f"Fixed contact threshold: {FIXED_ACTIVITY_THRESHOLD_LSB:g} LSB")
    print(f"Contact establishment summary trials per group: {args.contact_summary_n}")
    print(
        "Contact stability: "
        f"window>={CONTACT_STABILITY_WINDOW_MS:g} ms, "
        f"changed_taxels<={CONTACT_STABILITY_MAX_CHANGED_TAXELS}"
    )
    print(f"Gaussian blur: {args.gaussian_blur}, sigma={args.gaussian_sigma:g} cells")
    print(f"Saved summary: {summary_path}")
    print(f"Saved establishment summary: {establishment_summary_path}")
    print(f"Saved per-trial mask matrices: {len(matrix_paths)} files under {args.force_output_dir / 'process_csv' / 'mask_csv'}")
    print(f"Saved group frequency matrices: {len(frequency_paths)} files under {args.force_output_dir / 'process_csv' / 'group_frequency_csv'}")
    print(f"Saved per-trial mask figures: {len(per_trial_figures)} files under {args.force_output_dir / 'png' / 'contact_masks'}")
    if failure_path:
        print(f"Saved failures: {failure_path}")
    if frequency_figure_path:
        print(f"Saved contact frequency figure: {frequency_figure_path}")
    if coverage_figure_path:
        print(f"Saved contact coverage figure: {coverage_figure_path}")
...
def run_force_rise_pipeline(args: argparse.Namespace) -> None:
    ...
    figure_path = plot_force_rise_summary(selected, args.force_output_dir, force_levels)
    ...
    long_tail_path = plot_force_long_tail_broken(long_tails, args.force_output_dir, force_levels)
...
def main() -> None:
    ...
    if args.mode == "slip":
        raise ValueError(
            "This is the contact-mask script. Use 08_tactile_slip_pipeline.py for section 8/test8 outputs."
        )
    run_contact_mask_pipeline(args)
