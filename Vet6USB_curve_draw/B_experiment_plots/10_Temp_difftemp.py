...
A_calibration_plots/06_Temperature_Calibration.py:
...
--- New (2026-05-13): Figure A curve-count filter ---
- PLOT_A_NUM_HEATING_CURVES / PLOT_A_NUM_COOLING_CURVES macros control how
  many heating and cooling curves are shown per point group in Figure A
  (the temperature-time-curve plot) to reduce cross-group overlap.
- Selection is based on the measured 10%-90% response-window average slope,
  dT/dt|_{t=0} = ΔT * (α/τ_fast + (1-α)/τ_slow), which naturally orders
  P0 (outermost) > P1 (middle) > P2 (innermost) thanks to their different
  τ_fast scales (~0.1s / ~0.4s / ~1.2s).
- Other figures and all CSV outputs are NOT affected by this filter.
"""
...
from plot_style import COLORS, apply_measurement_style, save_chinese_png  # noqa: E402


CALIBRATION_SCRIPT = PLOT_ROOT / "A_calibration_plots" / "06_Temperature_Calibration.py"
DATA_ROOT = REPO_ROOT / "Workspace" / "B_experiments" / "B10_temperature_difftemp"
DEFAULT_POINTS = ("0_cold", "1_cold", "3_cold", "P0", "P1", "P2", "P3")
OUTPUT_DIR = PLOT_ROOT / "result_display" / "B_experiment" / "test10_temp_difftemp"

INFER_JAW0_FROM_INITIAL_LSB = 0
...
# === Crowded visualization filter (macro switches) ==========================
# Limit how many heating / cooling curves are shown per point group in
# the temperature-time plot and P0/P1/P2 signed-normalized overlay to reduce
# cross-group overlap. Other figures and CSV outputs are NOT affected.
# Set either count to None to show all curves for that direction.
PLOT_A_NUM_HEATING_CURVES: int | None = 3   # heating curves per point group
PLOT_A_NUM_COOLING_CURVES: int | None = 2   # cooling curves per point group
# Before the heating top/spread selection, drop the lowest-temperature heating
# curves per group. "Lowest" means smallest steady-state temperature among
# heating curves. These three switches are independent by group.
PLOT_A_DROP_LOW_HEATING_CURVES_P0: int = 2
PLOT_A_DROP_LOW_HEATING_CURVES_P1: int = 1
PLOT_A_DROP_LOW_HEATING_CURVES_P2: int = 1
# P1-only high-temperature heating trim. This is useful when the hottest P1
# heating curves crowd or cross the overlay after the low-temperature trim.
PLOT_A_DROP_HIGH_HEATING_CURVES_P1: int = 1
# Selection strategy:
#   "top"    : top-N by |initial slope| — emphasises the outer envelope,
#              best for showing P0 outermost / P1 middle / P2 innermost.
#   "spread" : N evenly-spaced samples across the |initial slope| range —
#              best for showing the within-group spread.
PLOT_A_SELECTION_MODE: str = "top"
PLOT_A_RISE_LOW_FRACTION: float = 0.10
PLOT_A_RISE_HIGH_FRACTION: float = 0.90
# ============================================================================
...
def infer_temperature_from_raw_lsb(raw_lsb: float, cold_junction_c: float) -> float:
    tc_mv = raw16_to_thermocouple_mv(raw_lsb)
    cold_mv = thermocouple_mv(int(round(cold_junction_c)))
    return float(temperature_from_mv(tc_mv + cold_mv))
...
def denoise_temperature_for_fit(T_c: np.ndarray, is_cooling: bool) -> np.ndarray:
    if not is_cooling:
        return T_c
    window = min(COOLING_DENOISE_WINDOW, T_c.size)
    return moving_average(T_c, window)
...
def load_and_process_trial(
    ...
    infer_jaw0_from_initial_lsb: bool,
    ...
    initial_raw_signal_lsb = float(raw_signal[0])
    ...
    tc_mv = np.asarray([raw16_to_thermocouple_mv(value) for value in corrected_raw16], dtype=float)
    raw_tail = T_c[-tail_n:]
    is_cooling = float(np.mean(raw_tail)) < float(T_c[0])
    T_fit_c = denoise_temperature_for_fit(T_c, is_cooling)
    tail = T_fit_c[-tail_n:]
    smooth_tail = moving_average(tail, min(SMOOTH_WINDOW, tail_n))
    steady_c = float(np.mean(tail))

    # Keep filename-based t_jaw0_c for beta / consistency diagnostics unless
    # the initial raw signal LSB is explicitly used to infer the jaw baseline.
    if infer_jaw0_from_initial_lsb:
        t_jaw0_c = infer_temperature_from_raw_lsb(initial_raw_signal_lsb, cold_junction_c)
    else:
    ...
    t_jaw0_for_norm = float(T_fit_c[0])
    T_hat = normalized_response(T_fit_c, t_jaw0_for_norm, steady_c)
    ...
    if metadata.t_liquid_c is not None and metadata.t_jaw0_c is not None:
        jaw_label_c = t_jaw0_c if infer_jaw0_from_initial_lsb else metadata.t_jaw0_c
        label = (
            f"T_liquid={metadata.t_liquid_c:g}°C, "
            f"T_jaw0={jaw_label_c:g}°C"
        )
    else:
    ...
    return TemperatureTrial(
        ...
        total_mv=total_mv,
        baseline_c=float(np.mean(T_fit_c[:baseline_n])),
        steady_c=steady_c,
...
def fit_normalized_response(trial: TemperatureTrial, max_points: int = NORMALIZED_FIT_POINTS) -> NormalizedFit:
    """
    Fit the normalised response T_hat with a double-exponential model:

        T_hat(t) = 1 - A·exp(-t/τ_fast) - B·exp(-t/τ_slow)
    ...
    t_fit = t_s[fit_start_index:]   # absolute time from contact — NO re-zeroing
    y_fit = T_hat[fit_start_index:]
    fit_mask = np.isfinite(t_fit) & np.isfinite(y_fit)
    ...
    if t_fit.size > max_points:
        indices = np.unique(np.linspace(0, t_fit.size - 1, max_points).round().astype(int))
        t_fit = t_fit[indices]
        y_fit = y_fit[indices]

    max_time = max(float(t_fit[-1]), 1.0)
    alpha_grid = np.linspace(0.05, 0.95, 19)
    tau_fast_grid = np.geomspace(0.05, max(0.2, max_time / 2.0), 40)
    tau_slow_grid = np.geomspace(0.2, max(2.0, max_time * 3.0), 55)
    best: tuple[float, float, float] | None = None
    best_sse = float("inf")

    for alpha in alpha_grid:
        for tau_fast in tau_fast_grid:
            valid_slow = tau_slow_grid[tau_slow_grid > tau_fast]
            if not valid_slow.size:
                continue
            exp_fast = np.exp(-t_fit / tau_fast)
            for tau_slow in valid_slow:
                pred = 1.0 - alpha * exp_fast - (1.0 - alpha) * np.exp(-t_fit / tau_slow)
                sse = float(np.sum((y_fit - pred) ** 2))
                if sse < best_sse:
                    best_sse = sse
                    best = (float(alpha), float(tau_fast), float(tau_slow))

    if best is None:
        return _fallback()

    alpha, tau_fast, tau_slow = best
    ...
    return NormalizedFit(
        alpha=alpha,
        tau_fast_s=min(tau_fast, tau_slow),
        tau_slow_s=max(tau_fast, tau_slow),
        R2=R2,
        t_fit_start_s=float(t_s[fit_start_index]),
        beta=beta_from_temperatures(trial.t_jaw0_c, trial.steady_c, trial.t_liquid_c),
        consistency=consistency_status(trial.t_jaw0_c, trial.steady_c, trial.t_liquid_c),
        popt=(alpha, min(tau_fast, tau_slow), max(tau_fast, tau_slow)),
    )
...
def filter_trials_by_consistency(trials: list[TemperatureTrial]) -> tuple[list[TemperatureTrial], list[dict[str, str]]]:
    kept: list[TemperatureTrial] = []
    filtered: list[dict[str, str]] = []
    for trial in trials:
        ...
        if norm_fit.consistency in {"ok_heat", "ok_cold"}:
            kept.append(trial)
            continue
        filtered.append(
            {
                "point": trial.path.parent.name,
                "file": trial.path.name,
                "reason": f"temperature consistency {norm_fit.consistency}",
            }
        )
    return kept, filtered
...
# ---------------------------------------------------------------------------
# Figure A curve-selection helpers (NEW)
# ---------------------------------------------------------------------------

def _first_fraction_crossing_time(t_s: np.ndarray, response: np.ndarray, fraction: float) -> float:
    """Return the first linearly interpolated time where response reaches fraction."""
    ...
    crossings = np.flatnonzero(response >= target)
    ...
    left = right - 1
    y0 = float(response[left])
    y1 = float(response[right])
    x0 = float(t_s[left])
    x1 = float(t_s[right])
    if not np.isfinite(y0 + y1 + x0 + x1) or abs(y1 - y0) < 1e-12:
        return x1
    ratio = (target - y0) / (y1 - y0)
    return float(x0 + np.clip(ratio, 0.0, 1.0) * (x1 - x0))


def compute_response_window_slope_c_per_s(
    trial: TemperatureTrial,
    low_fraction: float = PLOT_A_RISE_LOW_FRACTION,
    high_fraction: float = PLOT_A_RISE_HIGH_FRACTION,
) -> float:
    """
    Average signed slope across the measured response-time window.

    The filter ranks curves by the time needed to move from low_fraction to
    high_fraction of the final temperature change. This uses the measured
    curve, so overshoot does not dominate the metric the way the fitted t=0
    derivative can. Positive means heating and negative means cooling.
    """
    t_s = np.asarray(trial.t_s, dtype=float)
    T_c = np.asarray(trial.T_c, dtype=float)
    delta_T = float(trial.steady_c - trial.baseline_c)
    if t_s.size < 3 or T_c.size != t_s.size or not np.isfinite(delta_T) or abs(delta_T) < 1e-12:
        return float("nan")

    lo = float(min(low_fraction, high_fraction))
    hi = float(max(low_fraction, high_fraction))
    if not (0.0 <= lo < hi <= 1.0):
        raise ValueError("Response-window fractions must satisfy 0 <= low < high <= 1")

    finite = np.isfinite(t_s) & np.isfinite(T_c)
    t_s = t_s[finite]
    T_c = T_c[finite]
    if t_s.size < 3:
        return float("nan")

    response = (T_c - trial.baseline_c) / delta_T
    response = moving_average(response, min(SMOOTH_WINDOW, response.size))
    response = np.maximum.accumulate(np.clip(response, 0.0, 1.0))

    t_low = _first_fraction_crossing_time(t_s, response, lo)
    t_high = _first_fraction_crossing_time(t_s, response, hi)
    if not (np.isfinite(t_low) and np.isfinite(t_high)) or t_high <= t_low:
        rc = trial.fit
        if rc is not None and np.isfinite(rc.tau_s) and rc.tau_s > 0:
            return delta_T / rc.tau_s
        return delta_T

    return float(delta_T * (hi - lo) / (t_high - t_low))


def compute_initial_slope_c_per_s(trial: TemperatureTrial) -> float:
    """
    Backward-compatible name for older callers.
    Returns the measured response-window average slope in degC/s.
    """
    return compute_response_window_slope_c_per_s(trial)


PLOT_A_DROP_LOW_HEATING_CURVES_BY_POINT = {
    "P0": PLOT_A_DROP_LOW_HEATING_CURVES_P0,
    "P1": PLOT_A_DROP_LOW_HEATING_CURVES_P1,
    "P2": PLOT_A_DROP_LOW_HEATING_CURVES_P2,
}

PLOT_A_DROP_HIGH_HEATING_CURVES_BY_POINT = {
    "P1": PLOT_A_DROP_HIGH_HEATING_CURVES_P1,
}


def low_heating_drop_count_for_point(point_name: str) -> int:
    value = PLOT_A_DROP_LOW_HEATING_CURVES_BY_POINT.get(point_name, 0)
    if value is None:
        return 0
    return max(0, int(value))


def high_heating_drop_count_for_point(point_name: str) -> int:
    value = PLOT_A_DROP_HIGH_HEATING_CURVES_BY_POINT.get(point_name, 0)
    if value is None:
        return 0
    return max(0, int(value))


def heating_temperature_rank_c(trial: TemperatureTrial) -> float:
    """Rank heating curves by target liquid temperature, falling back to T_ss."""
    if trial.t_liquid_c is not None and np.isfinite(trial.t_liquid_c):
        return float(trial.t_liquid_c)
    return float(trial.steady_c)


def select_trials_for_temperature_plot(
    trials: list[TemperatureTrial],
    n_heating: int | None,
    n_cooling: int | None,
    mode: str = "top",
    point_name: str = "",
) -> tuple[list[TemperatureTrial], list[tuple[str, float]]]:
    """
    Split trials into heating / cooling by the sign of the response-window
    slope, rank each direction by |slope|, then keep the top N (or N
    evenly-spread samples) per direction.  Preserves the input ordering
    in the returned list.

    Returns (selected_trials, dropped_log) where dropped_log is a list of
    (filename, slope) for trials filtered out — for diagnostic printing.
    """
    drop_low_heating = low_heating_drop_count_for_point(point_name)
    drop_high_heating = high_heating_drop_count_for_point(point_name)
    if n_heating is None and n_cooling is None and drop_low_heating <= 0 and drop_high_heating <= 0:
        return list(trials), []

    heating: list[tuple[TemperatureTrial, float]] = []
    cooling: list[tuple[TemperatureTrial, float]] = []
    for trial in trials:
        slope = compute_response_window_slope_c_per_s(trial)
        if not np.isfinite(slope) or slope == 0.0:
            continue
        (heating if slope > 0 else cooling).append((trial, slope))

    dropped_ids: set[int] = set()
    if drop_low_heating > 0 and heating:
        low_heating_dropped = sorted(heating, key=lambda x: heating_temperature_rank_c(x[0]))[:drop_low_heating]
        dropped_ids.update(id(t) for t, _ in low_heating_dropped)
        heating = [(t, slope) for t, slope in heating if id(t) not in dropped_ids]
    if drop_high_heating > 0 and heating:
        high_heating_dropped = sorted(heating, key=lambda x: heating_temperature_rank_c(x[0]), reverse=True)[:drop_high_heating]
        dropped_ids.update(id(t) for t, _ in high_heating_dropped)
        heating = [(t, slope) for t, slope in heating if id(t) not in dropped_ids]

    def _pick(
        items: list[tuple[TemperatureTrial, float]],
        n: int | None,
        key,
        reverse: bool = True,
    ) -> list[TemperatureTrial]:
        if n is None or n >= len(items):
            return [x[0] for x in items]
        ranked = sorted(items, key=key, reverse=reverse)
        if mode == "spread" and n > 1:
            idx = np.unique(np.linspace(0, len(ranked) - 1, n).round().astype(int))
            return [ranked[i][0] for i in idx]
        return [x[0] for x in ranked[:n]]

    picked_heating = _pick(heating, n_heating, key=lambda x: heating_temperature_rank_c(x[0]), reverse=True)
    picked_cooling = _pick(cooling, n_cooling, key=lambda x: abs(x[1]), reverse=True)
    kept_ids = {id(t) for t in picked_heating + picked_cooling}
    if n_heating is None and n_cooling is None:
        kept_ids = {id(t) for t in trials if id(t) not in dropped_ids}
    selected = [t for t in trials if id(t) in kept_ids]
    dropped = [
        (t.path.name, compute_response_window_slope_c_per_s(t))
        for t in trials
...
# ---------------------------------------------------------------------------
# Plotting — temperature time curves (with Figure A filter)
# ---------------------------------------------------------------------------

def plot_temperature_curves(trials: list[TemperatureTrial], cold_junction_c: float, point_name: str) -> plt.Figure:
    # --- Apply Figure A filter: limit heating/cooling curves per group ---
    plot_trials, _ = select_trials_for_temperature_plot(
        trials,
        n_heating=PLOT_A_NUM_HEATING_CURVES,
        n_cooling=PLOT_A_NUM_COOLING_CURVES,
        mode=PLOT_A_SELECTION_MODE,
    ...
    # Title now reflects the filter state when active
    filter_note = ""
    if PLOT_A_NUM_HEATING_CURVES is not None or PLOT_A_NUM_COOLING_CURVES is not None:
        nh = "all" if PLOT_A_NUM_HEATING_CURVES is None else str(PLOT_A_NUM_HEATING_CURVES)
        nc = "all" if PLOT_A_NUM_COOLING_CURVES is None else str(PLOT_A_NUM_COOLING_CURVES)
        filter_note = f"  [top-|dT/dt|₀ heating×{nh}, cooling×{nc}]"
    ax.set_title(f"{point_name} Temperature Time Curves (Different Liquid Temperatures){filter_note}")
...
def plot_difference_model(trials: list[TemperatureTrial], point_name: str) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(7.4, 4.6))
    colors = [COLORS["blue"], COLORS["teal"], COLORS["green"], COLORS["orange"], COLORS["red"], COLORS["purple"]]
    for idx, trial in enumerate(trials):
        color = colors[idx % len(colors)]
        residual = trial.T_c - trial.T_fit_c
        smooth_window = min(SMOOTH_WINDOW, residual.size)
        residual_smooth = moving_average(residual, smooth_window)
        fit = trial.fit or RCFit()
        label = trial.label if not np.isfinite(fit.R2) else f"{trial.label}, R2={fit.R2:.3f}"
        ax.plot(trial.t_s, residual, color=color, linewidth=0.55, alpha=0.18)
        ax.plot(trial.t_s, residual_smooth, color=color, linewidth=1.35, alpha=0.92, label=label)

    ax.axhline(0.0, color=COLORS["gray"], linewidth=0.9, linestyle="--")
    ax.set_xlabel("Time t [s]")
    ax.set_ylabel("Measured - model [degC]")
    ax.set_title(f"{point_name} Difference Model Residuals")
    ...
    for idx, trial in enumerate(trials):
        ...
        if fit.popt is not None:
            ...
            label = (
                f"{trial.label}\n"
                f"  A={A:.2f}, B={B:.2f}, "
                f"τ_f={fit.tau_fast_s:.2f}s, τ_s={fit.tau_slow_s:.2f}s, "
                f"R²={fit.R2:.3f}"
            )
        else:
...
def plot_signed_normalized_response(trials: list[TemperatureTrial], point_name: str) -> plt.Figure:
    """Plot normalized response with physical temperature direction preserved."""
    fig, ax = plt.subplots(figsize=(7.8, 5.0))
    colors = [COLORS["blue"], COLORS["teal"], COLORS["green"], COLORS["orange"], COLORS["red"], COLORS["purple"]]
    ...
    for idx, trial in enumerate(trials):
        ...
        delta_c = float(trial.steady_c - trial.T_fit_c[0])
        sign = 1.0 if delta_c >= 0.0 else -1.0
        direction = "heating" if sign > 0 else "cooling"
        y_signed = sign * y_plot
        y_smooth = moving_average(y_signed, min(SMOOTH_WINDOW, y_signed.size))
        step = max(1, int(np.ceil(t_plot.size / 120)))
        label = (
            f"{trial.label}\n"
            f"  {direction}, T0={trial.T_fit_c[0]:.1f}C -> Tss={trial.steady_c:.1f}C"
        )

        ax.scatter(t_plot[::step], y_signed[::step], s=8, alpha=0.22, color=color)
        ax.plot(t_plot, y_smooth, color=color, linewidth=1.45, label=label)

        if t_plot.size >= 4:
            arrow_end = max(2, min(t_plot.size - 1, int(round(t_plot.size * 0.72))))
            arrow_start = max(0, arrow_end - max(2, int(round(t_plot.size * 0.08))))
            ax.annotate(
                "",
                xy=(float(t_plot[arrow_end]), float(y_smooth[arrow_end])),
                xytext=(float(t_plot[arrow_start]), float(y_smooth[arrow_start])),
    ...
    ax.axhline(-1.0, color=COLORS["gray"], linewidth=0.8, linestyle="--")
    ax.set_xlabel("Time since contact [s]")
    ax.set_ylabel("Signed normalized response")
    ax.set_title(f"{point_name} Signed Normalized Response\nHeating -> +1, cooling -> -1", fontsize=10)
    ax.legend(frameon=False, fontsize=6.5, loc="best")
...
def plot_signed_normalized_overlay_by_k(results: list[dict[str, object]]) -> plt.Figure:
    ...
    for point_name in ("P0", "P1", "P2"):
        result = next((item for item in results if item["point"] == point_name), None)
        if result is None:
            continue
        trials = result.get("plot_trials", result["trials"])
        color = group_colors[point_name]
        ...
        for trial in trials:
            ...
            y_signed = sign * y_plot
            y_smooth = moving_average(y_signed, min(SMOOTH_WINDOW, y_signed.size))
            label = layer_labels[point_name] if not group_label_added else None
            ax.plot(t_plot, y_smooth, color=color, linewidth=1.35, alpha=0.72, label=label)
            ...
            if t_plot.size >= 4:
                ...
                ax.annotate(
                    ...
                    xy=(float(t_plot[arrow_end]), float(y_smooth[arrow_end])),
                    xytext=(float(t_plot[arrow_start]), float(y_smooth[arrow_start])),
                    arrowprops={"arrowstyle": "->", "color": color, "lw": 1.0, "shrinkA": 0, "shrinkB": 0, "alpha": 0.72},
                )
...
def plot_tau_vs_k(results: list[dict[str, object]]) -> plt.Figure:
    ...
    layer_labels = ["Layer 0", "Layer 1", "Layer 3"]
    x_positions = np.arange(len(point_names), dtype=float)
    rng_offsets = np.linspace(-0.10, 0.10, 9)
    ...
    for ax, attr in (
        ...
        for idx, point_name in enumerate(point_names):
            ...
            if result is None:
                continue
            values = [
                getattr(trial.normalized_fit or NormalizedFit(), attr)
                for trial in result["trials"]
                if np.isfinite(getattr(trial.normalized_fit or NormalizedFit(), attr))
            ]
            if not values:
                continue
            values_arr = np.asarray(values, dtype=float)
            offsets = np.resize(rng_offsets, values_arr.size)
            x = np.full(values_arr.size, x_positions[idx], dtype=float) + offsets
            ax.scatter(x, values_arr, color=group_colors[point_name], alpha=0.80, s=28, edgecolors="none")
            mean_value = float(np.mean(values_arr))
            std_value = float(np.std(values_arr))
            ax.hlines(mean_value, x_positions[idx] - 0.22, x_positions[idx] + 0.22, color=group_colors[point_name], linewidth=1.8)
            if values_arr.size > 1 and np.isfinite(std_value) and std_value > 0:
                ax.errorbar(
                    [x_positions[idx]],
                    [mean_value],
                    yerr=[std_value],
                    fmt="none",
                    ecolor=group_colors[point_name],
                    elinewidth=1.2,
                    capsize=3,
                    alpha=0.9,
                )
    ...
    axes[0].set_ylabel("Fast time constant")
    axes[1].set_ylabel("Slow time constant")
    axes[1].set_ylim(10.0, 27.5)
    for ax in axes:
...
def save_point_figure(fig: plt.Figure, output_dir: Path, point_name: str, stem: str) -> Path:
    figure_dir = output_dir / "png" / point_name
...
def save_root_png_figure(fig: plt.Figure, output_dir: Path, stem: str) -> Path:
        ...
        rows = [
            {
                ...
                "temperature_fit_c": round(float(temp_fit), 6),
            ...
            for t, temp, temp_fit, t_hat, raw, corrected, tc_mv, total_mv in zip(
                trial.t_s,
                trial.T_c,
                trial.T_fit_c,
...
def trial_process_rows(
    ...
    """Build CSV rows for processing parameters and intermediate quantities."""
    rows: list[dict[str, object]] = []
    for trial in trials:
        fit = trial.fit or RCFit()
        norm_fit = trial.normalized_fit or NormalizedFit()
        tail_n = max(1, min(trial.T_c.size, int(round(noise_tail_s * sampling_rate_hz))))
        cooling_denoise_applied = bool(np.any(np.abs(trial.T_fit_c - trial.T_c) > 1e-12))
        rows.append(
            {
                "point": trial.path.parent.name,
                "file": trial.path.name,
                ...
                "offset_raw16": trial.offset_raw16,
                "contact_threshold_raw16": trial.contact_threshold_raw16,
                "contact_rise_frames": trial.contact_rise_frames,
                "contact_index": trial.contact_index,
                "contact_time_s": trial.contact_time_s,
                "cooling_denoise_applied": cooling_denoise_applied,
                "cooling_denoise_window": COOLING_DENOISE_WINDOW if cooling_denoise_applied else 0,
                "raw_signal_first_adc": float(trial.raw_signal_adc[0]),
                "raw_signal_last_adc": float(trial.raw_signal_adc[-1]),
                "corrected_raw16_tail_mean": float(np.mean(trial.corrected_raw16[-tail_n:])),
                "thermocouple_mV_tail_mean": float(np.mean(trial.thermocouple_mv[-tail_n:])),
                "total_mV_tail_mean": float(np.mean(trial.total_mv[-tail_n:])),
                "T0_fit_c": fit.T_0_c,
                "norm_t_fit_start_s": norm_fit.t_fit_start_s,
            }
        )
    return rows


def trial_result_rows(trials: list[TemperatureTrial], cold_junction_c: float) -> list[dict[str, object]]:
    """Build CSV rows for experimental results and fitted model parameters."""
    rows: list[dict[str, object]] = []
    for trial in trials:
        fit = trial.fit or RCFit()
        norm_fit = trial.normalized_fit or NormalizedFit()
        rows.append(
            {
                ...
                "norm_R2": norm_fit.R2,
                "response_window_slope_c_per_s": compute_response_window_slope_c_per_s(trial),
            }
        )
    return rows
...
def clear_legacy_outputs(output_dir: Path) -> None:
    legacy_paths = [
        ...
        output_dir / "csv" / "10_all_points_temperature_summary.csv",
    ]
    legacy_paths.extend((output_dir / "csv").glob("10_*_temperature_summary.csv"))
    legacy_paths.extend((output_dir / "csv").glob("10_*_temperature_process.csv"))
    legacy_paths.extend((output_dir / "csv").glob("10_*_temperature_results.csv"))
    legacy_paths.extend((output_dir / "csv").glob("10_*_processing_failures.csv"))
    legacy_paths.extend((output_dir / "csv" / "traces").glob("10_*_temperature_trace.csv"))
    legacy_stems = (
    ...
    for figure_dir in (output_dir / "png", output_dir / "pdf"):
        ...
        legacy_paths.extend(figure_dir.glob("10_*_temperature_time_curves.*"))
        legacy_paths.extend(figure_dir.glob("10_*_tail_temperature_summary.*"))
        legacy_paths.extend(figure_dir.glob("10_*_normalized_double_exp_fit.*"))
        legacy_paths.extend(figure_dir.glob("10_*_difference_model.*"))
        legacy_paths.extend(figure_dir.glob("10_P0_P1_P2_signed_normalized_overlay.png"))
        legacy_paths.extend(figure_dir.glob("10_tau_fast_tau_slow_vs_k.png"))
    legacy_paths.extend((output_dir / "png" / "pun_normalized_have_signal").glob("10_*_signed_normalized_response.png"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build temperature-time curves for B10_temperature_difftemp.")
    ...
    parser.add_argument(
        "--infer-jaw0-from-initial-lsb",
        type=int,
        choices=(0, 1),
        default=INFER_JAW0_FROM_INITIAL_LSB,
        help="Use the initial raw signal LSB to infer T_jaw0_c when set to 1.",
    )
...
def process_point(args: argparse.Namespace, point_name: str, data_dir: Path) -> dict[str, object] | None:
    ...
    if not trials:
        print(f"Skipping {point_name}: no readable CSV files.")
        return {
            "point": point_name,
            "data_dir": data_dir,
            "trials": [],
    ...
    trials, consistency_filtered = filter_trials_by_consistency(trials)
    consistency_filtered_count = len(consistency_filtered)
    failures.extend(consistency_filtered)
    if not trials:
        print(f"Skipping {point_name}: no CSV files passed ok_heat/ok_cold consistency filtering.")
        return {
            "point": point_name,
            "data_dir": data_dir,
            "trials": [],
    ...
    process_rows = trial_process_rows(
        trials,
    ...
    result_rows = trial_result_rows(trials, args.cold_junction_c)

    # --- Figure A: log which curves the filter kept/dropped ---
    plot_a_trials, plot_a_dropped = select_trials_for_temperature_plot(
        trials,
        n_heating=PLOT_A_NUM_HEATING_CURVES,
        n_cooling=PLOT_A_NUM_COOLING_CURVES,
        mode=PLOT_A_SELECTION_MODE,
    ...
    drop_high_heating = high_heating_drop_count_for_point(point_name)
    if (
        PLOT_A_NUM_HEATING_CURVES is not None
        or PLOT_A_NUM_COOLING_CURVES is not None
        or drop_low_heating > 0
        or drop_high_heating > 0
    ):
        nh = "all" if PLOT_A_NUM_HEATING_CURVES is None else PLOT_A_NUM_HEATING_CURVES
        nc = "all" if PLOT_A_NUM_COOLING_CURVES is None else PLOT_A_NUM_COOLING_CURVES
        print(
            f"Visualization filter [{point_name}]: kept {len(plot_a_trials)}/{len(trials)} curves "
            f"(heating hottest {nh}, cooling top {nc}, drop lowest heating {drop_low_heating}, "
            f"drop highest heating {drop_high_heating}, "
            f"mode={PLOT_A_SELECTION_MODE})"
        )
        for trial in plot_a_trials:
            slope = compute_response_window_slope_c_per_s(trial)
            print(f"  + kept    {trial.path.name}  (response-window slope {slope:+.2f} degC/s)")
        for fname, slope in plot_a_dropped:
            print(f"  - dropped {fname}  (response-window slope {slope:+.2f} degC/s)")

    fig_curves = plot_temperature_curves(trials, args.cold_junction_c, point_name)
    curves_path = save_point_figure(fig_curves, args.output_dir, point_name, f"10_{point_name}_temperature_time_curves")
    plt.close(fig_curves)

    fig_difference = plot_difference_model(trials, point_name)
    difference_path = save_point_figure(fig_difference, args.output_dir, point_name, f"10_{point_name}_difference_model")
    plt.close(fig_difference)

    fig_steady = plot_steady_temperature(trials, point_name)
    steady_path = save_point_figure(fig_steady, args.output_dir, point_name, f"10_{point_name}_tail_temperature_summary")
    plt.close(fig_steady)

    fig_normalized = plot_normalized_fits(trials, point_name)
    normalized_path = save_point_figure(fig_normalized, args.output_dir, point_name, f"10_{point_name}_normalized_double_exp_fit")
    plt.close(fig_normalized)

    fig_signed_normalized = plot_signed_normalized_response(trials, point_name)
    signed_normalized_path = save_point_figure(
        fig_signed_normalized,
        args.output_dir,
        point_name,
        f"10_{point_name}_signed_normalized_response",
    )
    plt.close(fig_signed_normalized)
    ...
    print(f"Infer T_jaw0 from initial LSB: {args.infer_jaw0_from_initial_lsb}")
    ...
    print(f"Consistency filter: kept {len(trials)} ok_heat/ok_cold trials; filtered {consistency_filtered_count}")
    ...
    print(f"Saved difference model figure: {difference_path}")
    print(f"Saved tail summary figure: {steady_path}")
    print(f"Saved normalized fit figure: {normalized_path}")
    print(f"Saved signed normalized response figure: {signed_normalized_path}")
    return {
        "point": point_name,
        "data_dir": data_dir,
        "trials": trials,
        ...
        "signed_normalized_path": signed_normalized_path,
        "plot_trials": plot_a_trials,
        "figure_paths": [curves_path, difference_path, steady_path, normalized_path, signed_normalized_path],
    }
...
def main() -> None:
    ...
    results = []
    all_process_rows: list[dict[str, object]] = []
    ...
    for point_name, data_dir in point_dirs:
        ...
        all_process_rows.extend(result["process_rows"])
    ...
    all_process_path = args.output_dir / "csv" / "10_all_points_temperature_process.csv"
    all_results_path = args.output_dir / "csv" / "10_all_points_temperature_results.csv"
    all_failures_path = args.output_dir / "csv" / "10_all_points_processing_failures.csv"
    write_csv(all_process_path, all_process_rows)
    write_csv(all_results_path, all_result_rows)
    write_csv(all_failures_path, all_failures)

    overlay_points = [result for result in results if result["point"] in {"P0", "P1", "P2"}]
    if overlay_points:
        fig_overlay = plot_signed_normalized_overlay_by_k(overlay_points)
        overlay_path = save_root_png_figure(fig_overlay, args.output_dir, "10_P0_P1_P2_signed_normalized_overlay")
        plt.close(fig_overlay)

        fig_tau = plot_tau_vs_k(overlay_points)
        tau_path = save_root_png_figure(fig_tau, args.output_dir, "10_tau_fast_tau_slow_vs_k")
        plt.close(fig_tau)
        print(f"Saved signed normalized overlay figure: {overlay_path}")
        print(f"Saved tau-vs-k figure: {tau_path}")

    print("All points summary")
    print(f"Processed points: {', '.join(result['point'] for result in results)}")
    print(f"Saved combined process table: {all_process_path}")
    print(f"Saved combined results table: {all_results_path}")
    print(f"Saved combined failures table: {all_failures_path}")
