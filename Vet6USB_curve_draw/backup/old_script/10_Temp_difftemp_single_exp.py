...
Single-exponential normalized-fit — FIXED version.
...
FIX 1 (load_and_process_trial): T_hat now normalised against T_c[0] (the
       ...
       filename-metadata t_jaw0_c.

FIX 2 (fit_normalized_response — THIS VERSION):
       The original code did NOT optimise tau on the normalised data — it
       simply reused the RC-fit tau (which was fitted on t >= 2 s temperature
       data) and evaluated R².  This caused two failures:
         a) When the RC tau hit the grid boundary (e.g. 100 s), the normalised
            model 1-exp(-t/100) was nearly flat over the 19 s window → R² ≈ 0.
         b) Even with a reasonable RC tau, using it from t = 0 introduced a
            systematic bias because the RC fit deliberately skipped the first
            2 s of transient data.
       NOW: fit_normalized_response uses scipy.optimize.least_squares to
       independently optimise tau on the normalised data from t = 0, using the
       RC tau only as an initial guess.  This makes the normalised R² and tau
       self-consistent and physically meaningful.

FIX 3 (fit_normalized_response): fallback preserves beta / consistency.
"""
...
OUTPUT_DIR = PLOT_ROOT / "result_display" / "test10_temperature_curves" / "difftemp_single_exp"
...
NORMALIZED_FIT_START_SKIP_S = 0.0
...
# ---------------------------------------------------------------------------
# Single-exponential normalised fit bounds
# ---------------------------------------------------------------------------
TAU_MIN_S = 0.5        # physical lower bound — below this is noise
TAU_MAX_S = 60.0       # generous upper bound
TAU_INIT_DEFAULT_S = 5.0  # fallback if RC fit is unavailable
...
# ---------------------------------------------------------------------------
# RC (single exponential) fit — temperature space, skips first 2 s
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Single-exponential (normalized) fit — FIXED: independent tau optimisation
# ---------------------------------------------------------------------------

def single_exp_normalized(t_s: np.ndarray, tau_s: float) -> np.ndarray:
    return 1.0 - np.exp(-t_s / tau_s)


def _single_exp_residuals(params: np.ndarray, t: np.ndarray, y: np.ndarray) -> np.ndarray:
    """Residuals for least_squares: model - data."""
    (tau,) = params
    return (1.0 - np.exp(-t / tau)) - y


def fit_normalized_response(trial: TemperatureTrial, max_points: int = NORMALIZED_FIT_POINTS) -> NormalizedFit:
    """
    FIX 2: Independently optimise tau on normalised data using least_squares.

    Model:  T_hat(t) = 1 - exp(-t / tau)

    The RC-fit tau is used only as an initial guess.  If it is unavailable or
    hit a grid boundary (e.g. 100 s), a sensible default is used instead.
    """
    ...
    fit_start_index = int(np.searchsorted(t_s, NORMALIZED_FIT_START_SKIP_S, side="left"))
    fit_start_index = min(max(fit_start_index, 0), t_s.size - 1)
    ...
    # --- FIX 2: Use RC tau as initial guess, but optimise independently ---
    rc_fit = trial.fit or RCFit()
    tau_init = float(rc_fit.tau_s)
    # Guard against bad RC tau (NaN, <=0, or hitting grid boundary at 100 s)
    if not np.isfinite(tau_init) or tau_init <= 0.0 or tau_init > TAU_MAX_S * 0.95:
        tau_init = TAU_INIT_DEFAULT_S
    # Clamp to bounds
    tau_init = max(TAU_MIN_S + 0.01, min(tau_init, TAU_MAX_S - 0.01))
    ...
    tau_opt = float(result.x[0])
    pred = single_exp_normalized(t_fit, tau_opt)
...
# ---------------------------------------------------------------------------
# Plotting — normalized single-exponential fit
# ---------------------------------------------------------------------------

def plot_normalized_fits(trials: list[TemperatureTrial], point_name: str) -> plt.Figure:
    """
    Normalized-fit plot. The legend shows:
        T_liquid = XX°C : τ=…s, R²=…

    Model: T_hat(t) = 1 - exp(-t/τ).
    """
    ...
    for idx, trial in enumerate(trials):
        ...
        if fit.popt is not None:
            label = (
                f"{trial.label}\n"
                f"  τ={fit.tau_s:.2f}s, R²={fit.R2:.3f}"
            )
        else:
        ...
        if fit.popt is not None:
            ...
            ax.plot(dense_t, single_exp_normalized(dense_t, fit.popt), color=color, linewidth=1.5, label=label)
        else:
    ...
    ax.set_title(f"{point_name} Normalized Response — Single-Exponential Fit\n"
                 r"$\hat{T}(t) = 1 - e^{-t/\tau}$",
                 fontsize=10)
    ax.legend(frameon=False, fontsize=6.5, loc="lower right")
    ax.margins(x=0.02, y=0.08)
        ...
        rows.append(
            {
                ...
                # --- RC (single-exp) fit in temperature space ---
                "rc_tau_s": fit.tau_s,
                "T_inf_c": fit.T_inf_c,
                "rc_R2": fit.R2,
                "delta_T_steady_c": fit.delta_T_steady_c,
                "snr": fit.snr,
                # --- Single-exp normalized fit (independently optimised) ---
                "norm_tau_s": norm_fit.tau_s,
                "norm_R2": norm_fit.R2,
            }
        )
    return rows
...
def process_point(args: argparse.Namespace, point_name: str, data_dir: Path) -> dict[str, object] | None:
    ...
    normalized_path = save_figure(fig_normalized, os.fspath(args.output_dir), f"10_{point_name}_normalized_single_exp_fit")
